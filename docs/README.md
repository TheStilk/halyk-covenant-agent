...
| [architecture.md](architecture.md) | Пайплайн LangGraph, модули, metrics, formulas, LLM env |
...
2. Прочитать [usage.md](usage.md) (env, `LLM_*`, CLI).  
...
5. `uv run python main.py phase3` → **BATTLE DIAGNOSTICS** → `validate`.  
6. Ядро: `metrics.py` + `formula_engine.py` + `formula_reader.py` + `battle_diagnostics.py`.  
